...
| `taxme_goto_section` | `name` (string) | click a menu section by name (substring); returns its fields — cut at 60 like `taxme_get_fields`, and saying so with `truncated`/`total`. Landing there is checked: TaxMe refuses to open a section while the form still has errors, and the page it hands back instead is reported as an error naming the section you are really on — not as that section's fields under the name you asked for. Which section that is is decided by the menu entry the breadcrumb *names*, not by the requested name turning up somewhere in it: these names nest ("Wertschriften" sits inside "Wertschriftenverzeichnis"), so a substring test says yes on the wrong page. A breadcrumb that names no entry, or a shorter one than the entry that was clicked, is ambiguous and is not read as a refusal; a page with no menu on it at all is no return, and says so |
