...
| `taxme_menu` | — | left-menu sections + status of the open return. The menu is on every page of a return and on no other page, so a page carrying none is no return: that comes back as an error naming where the browser is, not as an empty list of sections. `taxme_goto_section`, `taxme_results` and `taxme_submit_return` say the same thing about the same page, rather than reporting a section they cannot find |
| `taxme_goto_section` | `name` (string) | click a menu section by name (substring); returns its fields — cut at 60 like `taxme_get_fields`, and saying so with `truncated`/`total`. Landing there is checked: TaxMe refuses to open a section while the form still has errors, and the page it hands back instead is reported as an error naming the section you are really on — not as that section's fields under the name you asked for. Which section that is is decided by the menu entry the breadcrumb *names*, not by the requested name turning up somewhere in it: these names nest both ways round ("Wertschriften" sits at the front of "Wertschriftenverzeichnis", "Einkünfte" at the end of "Übrige Einkünfte"), so a substring test says yes on the wrong page. A breadcrumb is a path and the page is the end of it, so it is read as a path: the entry it names is the one in the last of its steps that names any entry at all, and within that step the longest, since a longer name that fits contains every shorter one nested inside it. A breadcrumb that names no entry at all, or one that is a shortened form of the name that was clicked — an entry that name begins with — is ambiguous and is not read as a refusal; any other entry is a different page and is. A page with no menu on it at all is no return, and says so |
...
| `taxme_fill` | `values: [{target, value}]` | set fields — `target` = field `id` **or** a label/context substring; `value` must be text, a number or `true`/`false`; text→typed, radio→option value or label, checkbox→`true`/`false` (`ja`/`nein`, `1`/`0`, `on`/`off` are understood too). An option neither a radio group nor a dropdown has is refused with a list of the ones it does, rather than attempted. Every value is read back after it is set and compared with what was asked for, so a widget the portal re-renders and hands back unchanged — a radio left unanswered, a dropdown back on another option or on none — comes back as `ok: false` rather than as set. `fields_after` is cut at 60 too, and says so |
...
| `taxme_results` | — | read the *Ergebnisse* / Steuerberechnung of the open return. Reaching that section is a precondition, and it is settled the way `taxme_goto_section` settles it — by the menu entry the breadcrumb names — before a word of the page is read as a calculation: a portal that refused to open the section comes back as an error naming the page you are actually on, not as that page's text, and the word *Ergebnisse* appearing in its refusal banner does not change that. The calculation is then read from a heading the left-menu entry cannot be, since that entry is on **every** page of the return. And if the click landed somewhere carrying no menu — an expired session lands on the login form — that is reported as no return being open, not as a page with no calculation on it |

**Submit (gated)**

| Tool | Args | Purpose |
| --- | --- | --- |
| `taxme_submit_return` | `confirm` (bool) | **⚠️ DANGER — irreversible final submission** (*Abschluss → Steuererklärung einreichen*). Without `confirm: true` it only opens the *Abschluss* page and returns a **dry-run** naming in `would_click` the one button a confirmed call would press; **nothing is submitted**. Reaching that page is a precondition — with no submit button on it the call is refused instead of pressing whatever the current page offers. A page that already reports the return as filed (`already_submitted`) is refused as well, `confirm: true` and all: a confirmation that was on the page beforehand cannot prove anything about a second click. So is a page whose text could not be read at all (`page_unreadable`), for the same reason one step further back — a read that failed is not a page that said no — the submit button is then named as `not_clicked`, because `would_click` is a promise about the confirmed call and that call presses nothing. And a page carrying no menu is no return at all — the *Abschluss* entry links to the edit view, and a session that died in the meantime is bounced to the login form — which is said in those words instead of reporting the *Abschluss* page as merely not open, the shape a portal refusing it over a form error has. Only `confirm: true` actually files the return. |
